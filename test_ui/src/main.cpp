...
namespace helper
{
    ...
    static ::ORB_SLAM3::Settings::SettingDesc getDefaultSettingDesc()
    {
        ...
        if(!init)
        {
            ...
            settingDesc.imuInfo.cvTbc = static_cast<cv::Mat>(cv::Mat_<float>(4, 4) <<
...
int main(int argc, char** argv)
{
    ...
    if(argc < 4)
    {
        APP_ERROR("Usage: test_new_ui [Path to vocabulary] [Path to data folder] [Path to shader folder]");
        return -1;
    }
    ...
    try
    {
        ...
        auto settings = new ORB_SLAM3::Settings(helper::getDefaultSettingDesc());
