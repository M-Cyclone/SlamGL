	...
	: m_folder_name(folder)
{
...

	{
		...
		while (std::getline(file, line))
		{
			...
			std::getline(iss, word, ','); imu_data.time_stamp = std::stoull(word);
			std::getline(iss, word, ','); imu_data.w_x = std::stod(word);
			std::getline(iss, word, ','); imu_data.w_y = std::stod(word);
			std::getline(iss, word, ','); imu_data.w_z = std::stod(word);
			std::getline(iss, word, ','); imu_data.a_x = std::stod(word);
			std::getline(iss, word, ','); imu_data.a_y = std::stod(word);
			std::getline(iss, word, ','); imu_data.a_z = std::stod(word);
...
std::pair<std::vector<ImgData>, std::vector<ImuData>> EuRoCDataLoader::getNextData() const
{
	std::pair<std::vector<ImgData>, std::vector<ImuData>> res;

	size_t time_stamp = m_image_name_list[m_current_idx];
	std::string path = m_folder_name + "cam0/data/" + std::to_string(time_stamp) + ".png";
