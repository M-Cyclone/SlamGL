...
OrbslamKernel::OrbslamKernel(ORB_SLAM3::Settings*      settings,
                             ORB_SLAM3::ORBVocabulary* vocabulary)
    : m_orb_system(
          vocabulary,
          settings,
          static_cast<ORB_SLAM3::System::eSensor>(settings->getSensor()))
{}

Sophus::SE3f OrbslamKernel::track(const std::vector<ImgData>& imgs,
                                  const std::vector<ImuData>& imus)
{
    ...
    {
        imu_points.emplace_back(imu.a_x,
                                imu.a_y,
                                imu.a_z,
                                imu.w_x,
                                imu.w_y,
    ...
    Sophus::SE3f pose = m_orb_system.TrackMonocular(imgs[0].image,
                                                    imgs[0].time_stamp * 1e-9,
                                                    imu_points);
...
auto OrbslamKernel::getPointCloudVetices() -> std::vector<PointVertex>
{
    ...
    std::unordered_set<ORB_SLAM3::MapPoint*> local_mp_ust(local_mp.begin(),
                                                          local_mp.end());
    ...
    for (auto mp : local_mp_ust)
    {
        ...
        pvs.push_back({ glm::vec3(eigen_pos.x(), eigen_pos.y(), eigen_pos.z()),
    ...
    APP_INFO("Global map point count: {0}. Local map point count: {1}",
             local_mp.size(),
             all_mp.size());
...
auto OrbslamKernel::getKeyFramePoses() -> std::vector<Sophus::SE3f>
{
    ...
    std::sort(all_kfs.begin(),
              all_kfs.end(),
              [](const ORB_SLAM3::KeyFrame* kf1, const ORB_SLAM3::KeyFrame* kf2)
              {
